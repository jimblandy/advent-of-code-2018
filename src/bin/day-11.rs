...
fn power_3x3(ul: (usize, usize), serial: usize) -> i32 {
    cartesian_product(0..3, 0..3).map(move |d| power(add2(ul, d), serial)).sum()
}
...
fn main() {
    println!("{:?}", cartesian_product(1..=298, 1..=298).max_by_key(|&pt| power_3x3(pt, 18)));
    println!("{:?}", cartesian_product(1..=298, 1..=298).max_by_key(|&pt| power_3x3(pt, 42)));
    println!("{:?}", cartesian_product(1..=298, 1..=298).max_by_key(|&pt| power_3x3(pt, 7689)));
